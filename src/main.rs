...
use winapi::um::fileapi::{GetDiskFreeSpaceA, GetLogicalDrives};

const VERSION: &str = env!("CARGO_PKG_VERSION");
const DATETIME_FORMAT: &str = "%d.%m %H:%M:%S";
...
fn main() {
    ...
    let drive_letter_as_cstring = CString::new(drive_letter).unwrap();
    if unsafe {
        GetDiskFreeSpaceA(
            drive_letter_as_cstring.as_ptr(),
    ...
    let mut garbage_file_size = match how_to_fill {
        HowToFill::WriteAll => {
            get_input(format!("По сколько {} будет каждый файл?", &kind).as_str())
        }
        HowToFill::WriteByChunk => get_input(
            format!(
                "По сколько {} будет каждый чанк для заполнения каждого файла?",
                &kind
            )
            .as_str(),
        ),
    ...
    match how_to_fill {
        HowToFill::WriteByChunk => {
            while total_wrote_size < total_garbage_size {
                ...
                file_path.push_str(Local::now().format(DATETIME_FORMAT).to_string().as_str());
                ...
                print!(
                    "\r{}/{} {}",
                    total_wrote_size / coefficient,
                    total_garbage_size / coefficient,
                    &kind
                );
